import { Button, Col, Form, FormInstanceFunctions, FormItem, Loading, MessagePlugin, Row, Space, Upload } from "tdesign-vue-next";
                ...
                return option.hidden ? null : <Col span={option.span || 6}>
                    {slots[option.name] && slots[option.name]({ option }) ? slots[option.name]({ option }) : <t-form-item
                        ...
                        <Upload
                            ...
                            v-model={innerFormData.value[option.name]}
                            //@ts-ignore
                            readonly={(props.readonly || option.readonly) as unknown as boolean} disabled={props.disabled || option.disabled}
                            {...option.uploadProps}>
                        </Upload>
                    </t-form-item>
                    }
            ...
            return <Row gutter={[12, 24]}>
                ...
                }
                {
                    footerRender ? <FormItem style={{ flex: 1 }}>
                        {footerRender}
                    </FormItem> : !props.hideFooter && <Col span={Number(props.footerSpan)}> <FormItem style={`display: flex; justify-content: right;`}>
                        <Space>
                            <div>
                                {extralRender}
                            </div>
                            ...
                            {
                                !props.hideReset && <Button type="reset" theme="default">{props.resetText}</Button>
                            }
                        </Space>
                    </FormItem>
                    </Col>
