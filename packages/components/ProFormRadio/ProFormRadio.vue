...
import { ProFormRadioProps } from './types';
import { FormItem, TdFormItemProps, RadioGroup, Radio } from 'tdesign-vue-next';
...
<template>
    <component :is="IFormItem">
        ...
        <RadioGroup v-model="modelValue" :readonly="props.radioProps?.readonly || props.readonly"
            :disabled="props.radioProps?.disabled || props.disabled" v-bind="{...$attrs, ...props.radioProps}">
            <Radio :disabled="option.disabled" :value="option.value" v-for="option in options" :key="option.value">
