...
import { Data, FormValidateResult, Drawer } from 'tdesign-vue-next';
...
<template>
    <Drawer :confirm-loading="loading" @before-close="handleBeforeClose" @confirm="handleConfirm"
        v-bind="{ ...attrs, ...props.drawerProps, header: props.title }" v-model:visible="visible">
        ...
        <div style="overflow-x: hidden; padding-bottom: 24px;" v-loading="loading">
            <ProForm ref="proFormRef" :form-props="{ labelAlign: 'top' }" :hide-actions="true" v-model="formData"
                :options="props.options" :request="props.request">
