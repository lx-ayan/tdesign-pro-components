...
const DrawerForm = defineComponent(
    ...
    <T extends any>(props: DrawerFormProps<T>, {emit, slots, expose}) => {
        ...
        return () => <>
            <Drawer size={props.width as unknown as string} onConfirm={handleConfirm} onCancel={handleCancel} onClose={handleClose} onBeforeOpen={handleOpened} v-model:visible={innerVisible.value} {...props.drawerProps}>
                {{
                    //@ts-ignore
                    default: () => <ProForm {...props.formProps} ref="proFormRef" gap={24} options={(props.options || []) as unknown as any} request={request.value} hideFooter>
