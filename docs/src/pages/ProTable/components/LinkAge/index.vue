...
<template>
    <div>
        <t-typography-title level="h3"><span id="ProTableLinkAge">嵌套内容</span></t-typography-title>
        <ProTable  :dataActionsAlign="align" ref="proTableRef" v-model:selectData="selectData" selectAble selectType="multiple" :request
            :options="options">
            <template v-if="currentData?.length" #dataActions>
                <div class="user-card">
                    <ProFormRadio gap="12" :radio-props="{ gap: '12px' }" theme="tag" vertical="col" name="user"
                        :data="currentData" labelName="username" value-name="id">
            ...
            <template #actions>
                <div>
                    <ProFormRadio v-model="align"  name="position"
                        :data="[{ label: '左边', value: 'left' }, { label: '右边', value: 'right' }]">
